...
//go:embed expenses.polar
var expensesPolar string
...
func main() {
	...
	o, _ := oso.NewOso()
	o.RegisterClass(Expense{}, nil)
	o.RegisterClass(User(""), nil)
	o.LoadString(expensesPolar)

	expenses := make(map[int]Expense)
	expenses[1] = Expense{
		Amount:      500,
		Description: "coffee",
		SubmittedBy: User("alice@example.com"),
	}
	expenses[2] = Expense{
	...
	expenses[3] = Expense{
		Amount:      50000,
		Description: "flight",
		SubmittedBy: User("bhavik@example.com"),
	}
	app := App{expenses: expenses, oso: o}
	fmt.Println("server running at port 5050")
	http.Handle("/", &app)
